...
import { assignParcel, lookupParcel } from "./core/parcel.js";
 ...
 * 出所を隠さない方針なので明示する。
 */
export const RAIL_ATTRIBUTION = ["駅データ.jp"];
...
function byDepth(a, b) {
    return ((DEPTH[b.matchLevel] ?? 0) - (DEPTH[a.matchLevel] ?? 0) || b.score - a.score);
}
function prefCandidate(pref) {
    return {
        address: pref.name,
        lat: pref.lat ?? 0,
        lng: pref.lng ?? 0,
...
function cityCandidate(city, prefName, score, building) {
    return {
 ...
 */
function townCandidate(city, entry, prefName, score, building, hit, parcel) {
...
async function descend(fetcher, shards, entry, rest) {
    ...
    if (isResidential(entry)) {
        ...
        const hit = await lookupRsdt(fetcher, code, shards.rsdt[code] ?? [], townId(entry), block, rsdt);
    ...
    if (!nums)
        return none;
    const parcel = await lookupParcel(fetcher, code, shards.parcel[code] ?? [], townId(entry), nums);
    return { hit: null, parcel, building };
}
/** 一致軸のスコア（§7）。漢字 1.00 / カナ 0.90 / 編集距離1 0.60 */
const axisScore = (hit) => hit.axis === "exact" ? 1.0 : hit.axis === "kana" ? 0.9 : 0.6;
/** 市区町村がタイポで一致したときのスコア */
 ...
 */
function aliasHits(alias, index, key) {
    const out = [];
    let best = "";
    for (const k of Object.keys(alias)) {
        if (k && key.startsWith(k) && k.length > best.length)
            best = k;
    }
    if (!best)
        return out;
    for (const code of alias[best] ?? []) {
        ...
        if (rec)
            out.push({ record: rec, rest: key.slice(best.length) });
    }
    return out;
}
export function createGeocoder(options = {}) {
    const fetcher = options.fetcher ?? createHttpFetcher(options.baseUrl ?? DEFAULT_BASE_URL);
    ...
    const loadAlias = () => {
        if (!aliasPromise) {
            ...
            aliasPromise = fetcher.get("alias.json").then((f) => f, () => null);
        }
        return aliasPromise;
    };
    ...
    const loadShards = () => {
        if (!shardsPromise) {
            ...
            shardsPromise = fetcher.get("shards.json").then((f) => f, () => null);
        }
     ...
     */
    async function resolveTowns(shards, index, rest, prefName, allowed, prefCode, outerScore = 1, 
    ...
    exactRest) {
        ...
        for (const hit of hits) {
            ...
            if (allowed)
                entries = entries.filter((e) => allowed.has(townCode(e)));
            else if (prefCode) {
        ...
        if (entries.length > 1 && exactRest) {
            const exactly = entries.filter((e) => exactRest.startsWith(toExactKey(townName(e))));
            if (exactly.length)
                entries = exactly;
        }
        ...
        for (let i = 0; i < ranked.length && out.length < max; i++) {
            ...
            const { hit, parcel, building } = i < DESCEND_LIMIT
    ...
    return {
        async geocode(address) {
            ...
            const exactOf = (tail) => tail.length && exactWhole.length === toKey(address).length
            ...
            if (!cityHits.length) {
                ...
                if (alias) {
                    ...
                    if (cityHits.length)
                        cityScore = ALIAS_SCORE;
                }
            }
            const fuzzy = fuzzyPrefix(toKey(rest), index.candidates(pref?.code).map((e) => ({ key: e.key, value: e.value })), 8);
            if (fuzzy.length) {
                const seen = new Set(cityHits.map((h) => h.value.code));
                for (const m of fuzzy) {
                    if (seen.has(m.value.code))
                        continue;
            ...
            if (shards) {
                if (cityHits.length) {
                    ...
                    for (const [tail, hits] of byRest) {
                        ...
                        all.push(...(await resolveTowns(shards, index, tail, prefName, allowed, pref?.code, cityScore, exactOf(tail))));
                    }
                    if (all.length) {
                        return {
                    ...
                    const direct = await resolveTowns(shards, index, rest, prefName, null, pref?.code, 1, exactOf(rest));
                    if (direct.length) {
                        return {
            ...
            const seen = new Set();
            for (const h of cityHits) {
                if (seen.has(h.value.code))
                    continue;
                seen.add(h.value.code);
                const tail = parseTail(h.rest);
                scored.push({
                    candidate: cityCandidate(h.value, prefName ?? index.pref(h.value.pref)?.name, h.score, tail.building),
                    record: h.value,
                });
            }
        ...
        async listStations(query = {}) {
            ...
            if (prefCode !== null)
                stations = stations.filter((s) => s.pref === prefCode);
        ...
        async nearestStations(lat, lng, options = {}) {
            const attribution = [...RAIL_ATTRIBUTION];
        ...
        async nearestStationsByAddress(address, options = {}) {
            ...
            const near = await this.nearestStations(candidate.lat, candidate.lng, options);
            return {
