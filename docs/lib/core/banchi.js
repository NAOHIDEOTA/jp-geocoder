...
const BUILDING_HINT = /(ビル|ビルヂング|マンション|ハイツ|コーポ|アパート|レジデンス|タワー|プラザ|パレス|メゾン|荘|棟|館|号室|階|F$|Ｆ|ハウス|テラス|ガーデン|コート|ヴィラ|パーク)/u;
/** 番地として読める連なり */
const RUN = /^[0-9]+(?:\s*(?:丁目|丁|番地の|番地|番|号|条|地割|の|ノ|之|[-ー―‐−–—])\s*[0-9]*)*/u;
/**
 ...
 */
export function parseTail(input) {
    const s = kanjiNumeralsToArabic(input.normalize("NFKC")).replace(/[\s　]/gu, "");
    const m = RUN.exec(s);
    if (!m || !m[0]) {
...
export function looksLikeBuilding(s) {
    if (!s)
        return false;
    if (BUILDING_HINT.test(s))
        return true;
    // 数字のみなら部屋番号とみなす
    return /^[0-9]+$/u.test(s);
}
//# sourceMappingURL=banchi.js.map