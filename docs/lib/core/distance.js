...
export function allowedDistance(len) {
    return len <= 3 ? 0 : 1;
}
/**
 * 上限つきレーベンシュタイン距離。
...
export function typoDistance(input, candidate) {
    // 数字は完全一致のみ。「1丁目」と「2丁目」を距離1にしない
    if (digitRuns(input) !== digitRuns(candidate))
        return null;
    const max = allowedDistance(candidate.length);
    if (max === 0)
        return input === candidate ? 0 : null;
 ...
 */
export function fuzzyPrefix(input, entries, limit = 5) {
    const out = [];
    for (const e of entries) {
        ...
        if (max === 0)
            continue; // 短いキーは完全一致のみ。前方一致側で拾われる
        // 完全一致するものは前方一致側の仕事。
