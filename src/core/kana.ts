...
const SEION: Readonly<Record<string, string>> = {
  が: "か",
  ぎ: "き",
  ぐ: "く",
  げ: "け",
...
const ARCHAIC: Readonly<Record<string, string>> = {
  ゐ: "い",
  ゑ: "え",
  を: "お",
};
 ...
 */
export function toKanaKey(input: string): string {
...
export function toKanaLooseKey(input: string): string {
  let out = "";
  for (const c of toKanaKey(input)) out += SEION[c] ?? c;
  return out;
}
