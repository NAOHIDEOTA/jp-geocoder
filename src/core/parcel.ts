...
export interface ParcelFile {
  code: string;
  t: Record<string, ParcelTown>;
}

export interface ParcelHit {
  lat: number;
  lng: number;
  /** 一致した地番。枝番が無ければ 0（[1234, 0, 0] の形） */
...
export async function lookupParcel(
  ...
  machiazaId: string,
  nums: readonly [number, number, number]
): Promise<ParcelHit | null> {
  let file: ParcelFile;
  try {
    file = (await fetcher.get(partName(code, bounds, machiazaId))) as ParcelFile;
  } catch {
    return null;
  }
  ...
  let start = -1;
  for (let i = 0; i < n; i++) {
    acc += town.a[i] as number;
    if (acc === wantA) { start = i; break; }
    if (acc > wantA) return null;
  }
  ...
  for (let i = start; i < n; i++) {
    if (i > start && (town.a[i] as number) !== 0) break; // a が変わった
    if ((town.b[i] ?? 0) === wantB && (town.c[i] ?? 0) === wantC) { hit = i; break; }
  }
  ...
  let dy = 0, dx = 0;
  for (let i = 0; i <= hit; i++) {
    dy += town.y[i] as number;
    dx += town.x[i] as number;
  }
